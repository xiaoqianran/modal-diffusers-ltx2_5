...
async function testSinglePut() {
  ...
  assert.equal(calls[1].url, 'https://s3.test/input')
...
async function testMultipartPut() {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (calls.length === 1) {
      return json({
        ...
        headers: {},
        parts: [
          { part_number: 1, url: 'https://s3.test/p1' },
          { part_number: 2, url: 'https://s3.test/p2' },
          { part_number: 3, url: 'https://s3.test/p3' },
        ],
      }, 201)
    }
    if (String(url).startsWith('https://s3.test/p')) {
  ...
  const puts = calls.filter(call => call.url.startsWith('https://s3.test/p'))
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (calls.length === 1) {
      return json({
        ...
        mode: 'single',
        method: 'PUT',
        url: 'https://s3.test/fail',
        headers: {},
        parts: [],
      }, 201)
    }
    if (String(url) === 'https://s3.test/fail') {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (calls.length === 1) {
      return json({
        ...
        mode: 'single',
        method: 'PUT',
        url: 'https://s3.test/done',
        headers: {},
        parts: [],
      }, 201)
    }
    if (String(url) === 'https://s3.test/done') return new Response('ok', { status: 200 })
...
async function testFinalizeRetriesWithoutReuploadingBytes() {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (calls.length === 1) {
      return json({
        ...
        mode: 'single',
        method: 'PUT',
        url: 'https://s3.test/retry',
        headers: {},
        parts: [],
      }, 201)
    }
    if (String(url) === 'https://s3.test/retry') return new Response('ok', { status: 200 })
    finalizeAttempts += 1
  ...
  assert.equal(calls.filter(call => call.url === 'https://s3.test/retry').length, 1)
