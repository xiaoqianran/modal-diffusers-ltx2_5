...
# requirements.txt changes. Editing backend/*.py no longer rebuilds Torch/CUDA.
...
if REMOTE_GATEWAY:
    # Compatibility only. The default interactive path uses backend/api/local.py
...
class LTX25Worker:
    ...
    def load(self):
        ...
        from backend.runtime.engine import LTXGenerator
    ...
    def generate(self, job_id: str, request_payload: dict) -> dict:
        ...
        from backend.config import settings
        from backend.contracts import GenerateRequest, STILL_IMAGE_MODES
